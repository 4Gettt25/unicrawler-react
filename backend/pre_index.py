...
def process_page(pdf_path, pdf_file, page, page_number):
    try:
        ...
        logging.debug(f"Inserted page {page_number} of {pdf_file} into database")
    except Exception as e:
        logging.error(f"Error processing page {page_number} of {pdf_file}: {e}")
        connection.rollback()
        ...
        if pdf_file.endswith('.pdf'):
            ...
            try:
                ...
                for i, page in enumerate(pages):
                    process_page(pdf_path, pdf_file, page, i + 1)
            except Exception as e:
